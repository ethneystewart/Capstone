...
ax.set_title('Wave Displacement and Buoy Motion Over Time')
...
# Create meshgrid for plotting (time vs displacement)
time_plot = np.arange(0, sim_time, dt)  # Time array
wave_disp_sampled = wave_disp[::downsample_factor]  # Smoothed wave displacement for visualization

X, Y = np.meshgrid(time_plot, np.linspace(0, D, 100))  # Time vs. displacement range

# Initialize the line for animation
line, = ax.plot([], [], [], label='Wave Surface', color='b')
buoy, = ax.plot([], [], [], marker='o', color='r', markersize=10, label='Buoy')
...
def update(frame):
    ...
    for i in range(len(X)):
        Z[i, :] = wave_disp_sampled[frame] * np.sin(2 * np.pi * i / len(X))
    
    ax.clear()  # Clear previous frame
    ax.plot_surface(X, Y, Z, cmap='viridis', edgecolor='none', alpha=0.7)
    buoy.set_data([time_plot[frame], time_plot[frame]], [D / 2, D / 2])  # Buoy remains at constant height
    buoy.set_3d_properties([wave_disp_sampled[frame]], 'z')
...
ani = FuncAnimation(fig, update, frames=len(wave_disp_sampled), interval=100, blit=False)

# Save the animation as an MP4 file
ani.save('wave_buoy_animation.gif', writer='pillow', fps=30) # You can change the format if needed
