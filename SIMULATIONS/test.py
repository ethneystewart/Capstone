...
def simulate_hourly_power(wave_height, wave_period):
    """
    Simulate one hour of buoy response and power output.
    Returns:
      time, buoy displacement, buoy velocity, instantaneous power (Pout),
      total energy output (integral of power over time in Joules),
      and total energy in kWh.
    """
    ...
    # Compute total energy output over the simulated period (Joules)
    total_energy_joules = np.sum(Pout) * dt
    # Convert total energy to kilowatt-hours (1 kWh = 3.6e6 J)
    total_energy_kWh = total_energy_joules / 3.6e6

    return time, buoy_disp, buoy_vel, Pout, total_energy_joules, total_energy_kWh
...
for day, group in tqdm(df.groupby(df['date'].dt.date)):
    total_energy_day_kWh = 0

    for _, row in group.iterrows():
        wave_height = row['wave_height']
        wave_period = row['wave_period']
        time, buoy_disp, buoy_vel, Pout, energy_joules, energy_kWh = simulate_hourly_power(wave_height, wave_period)
        total_energy_day_kWh += energy_kWh

        hourly_data.append({
            'date': row['date'],
            'wave_height': wave_height,
            'wave_period': wave_period,
            'total_energy_kWh': energy_kWh  # Energy in kWh
        })
    ...
    daily_summary.append({
        'date': day,
        'total_energy_kWh': total_energy_day_kWh  # Total energy for the day in kWh
    })
...
# --- Output the Total Energy Data in kWh ---
print("Daily Total Energy Output (kWh):")
for summary in daily_summary:
    print(f"Date: {summary['date']}, Total Energy: {summary['total_energy_kWh']:.4f} kWh")
